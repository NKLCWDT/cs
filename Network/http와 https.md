...
> - Get v.s Post  
  ...
  반면, post의 경우에는 전송데이터를 body에 담아 전송한다. 그래서 눈에 안보이고 get보다 안전하다.(chrome 개발자 도구를 사용하면 확인이 가능하기에 암호화를 해야한다). 또 요청의 header에 content-type을 표시해줘야 한다.(content type은 보내는 type에 따라 달라진다 json형식으로 보낸다면 'application/json')
